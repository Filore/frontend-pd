...
export default function Leaderboard() {
  ...
  const list = leaderList.find(l => l.name === listCategory)?.rating ?? [];
  return [...list].sort((a, b) => (b.money - a.money) || a.name.localeCompare(b.name));
}, [leaderList, listCategory])
        ...
        "> 
          <ul>
            {actualList.map((user, index) => {
              const top3 = index < 3;
              return (
                <li key={`${user.name}-${user.money}-${index}`}>
